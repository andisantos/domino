...
def _run_domino(problem, n_slices, n_mixtures):
    ...
    val_dp = embed(val_dp, input_col="image", device=0)
    print("Embedding test images:")
    test_dp = embed(test_dp, input_col="image", device=0)
    # Init. Domino
    domino_config = {
    ...
    domino = DominoSlicer(**domino_config)
    # Fit on val-data
    print("Fitting on validation images:")
    domino.fit(
        data=val_dp, embeddings="clip(image)", targets="target", pred_probs="pred"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="domino evaluation script")
    parser.add_argument(
        "-s",
        "--slice",
        type=str,
    ...
    parser.add_argument("-k", "--k", type=int, help="number of clusters")
    parser.add_argument("-m", "--m", type=int, help="number of mixture components")
